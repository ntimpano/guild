...
# install-agents.sh - Installs Guild agents into Pi's global agent directory.
# Usage: ./install-agents.sh [--uninstall] [--dry-run]
...
function verify_pi_installed() {
    if [ ! -d "$HOME/.pi" ]; then
        error "Pi is not installed. Please install Pi first: https://opencode.ai/pi"
    fi
}

function verify_flint_installed() {
    if ! command -v flint &> /dev/null; then
        log "WARNING: Flint is not installed. Some agents (guild-init, guild-archivist) will have limited functionality."
        log "To install Flint: https://github.com/opencode-ai/flint"
        return 1
    fi
    return 0
}

function backup_existing_agents() {
    if [ -d "$PI_AGENTS_DIR" ] && [ -n "$(ls -A "$PI_AGENTS_DIR")" ]; then
        log "Backing up existing agents to $BACKUP_DIR"
        mkdir -p "$BACKUP_DIR"

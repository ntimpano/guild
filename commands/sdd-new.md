...
PERSISTENCE NOTE:
Sub-agents handle flint persistence automatically. Each phase saves with topic_key "sdd/$ARGUMENTS/{type}" via `flint_local_save`. Engram is NOT used — there is no other backend, no mode selection.

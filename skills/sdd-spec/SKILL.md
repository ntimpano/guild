...
├── This becomes a DELTA spec for that capability
└── Read the existing capability spec first — your delta modifies it
```
...
Compose the spec content and persist it in Step 5.
...
```
1. Locate the requirement in the current main spec for that domain
...
→ The archive step REPLACES the requirement in the main spec with your MODIFIED block

...
> **The ONLY persistence backend is flint.** Engram does NOT exist in this stack and never did. Never call `mem_save`, `mem_search`, `mem_get_observation`, or any `mem_*` tool. There is no opt-out, no alternative mode, no fallback.

## Backend

flint local SQLite via the `flint_local_*` MCP tools. Always available, always required.

| Capability | Behavior |
|------------|----------|
| Cross-session recovery | Yes |
| Compaction survival | Yes |
| Project scoping | Yes (auto-detected via `flint_project_*`) |
...
```
Persist: flint_local_save(title: "sdd/{change-name}/state", topic_key: "sdd/{change-name}/state", type: "architecture", scope: "{project}", content: "{yaml state}")
Recover: flint_local_recall(query: "sdd/{change-name}/state")
```

## Common Rules

- Every SDD phase that produces an artifact MUST persist it via `flint_local_save`. No exceptions.
...
- **Non-SDD task**: orchestrator searches flint, passes the relevant summary in the prompt; sub-agent saves discoveries via `flint_local_save`.
- **SDD phase with dependencies** (spec, design, tasks, apply, verify, archive): sub-agent reads required artifacts directly via `flint_local_recall`; sub-agent saves its artifact via `flint_local_save`.
...
bug fixes, or stable user preferences via flint:
...
Read these artifacts before starting (run in parallel):
 flint_local_recall(query: "sdd/{change-name}/{type-1}")
 flint_local_recall(query: "sdd/{change-name}/{type-2}")
